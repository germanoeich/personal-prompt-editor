...
export default function Home() {
  ...
  const openPromptInTab = useCallback(
    (prompt: Prompt) => {
  ...
  const closeTab = useCallback(
    (tabId: string) => {
      ...
      if (
        tab?.isDirty &&
        !confirm("This tab has unsaved changes. Close anyway?")
      ) {
        return;
      }
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === "s") {
        ...
        // Only save if there's an active tab with content
        if (activeTab && activeTab.content.length > 0) {
          // If it's a new prompt without a title, we need to prompt for one
          if (activeTab.isNew || !activeTab.title) {
            const title = prompt(
              "Enter a title for your prompt:",
              activeTab.title || "Untitled Prompt"
            );
            if (title) {
              handleSavePrompt(title);
            }
          } else {
            // Save existing prompt with current title
            handleSavePrompt(activeTab.title);
          }
        }
      }
    };
  ...
  }, [activeTab, handleSavePrompt]);

  // Load prompt functionality - now opens in tab
  const handlePromptLoad = useCallback(
    async (prompt: Prompt) => {
  ...
  const handlePromptDelete = useCallback(
    async (promptId: number) => {
      ...
      try {
        ...
        // Close any tabs that have this prompt open
        const tabsToClose = tabs.filter(tab => tab.promptId === promptId);
    ...
    async (newTitle: string) => {
        ...
        prev.map((tab) => {
          if (tab.id === activeTabId) {
            ...
            return {
              ...tab,
              title: newTitle,
              isDirty: tab.isDirty || (tab.isNew && titleChanged),
            };
          }
          return tab;
        })
      ...
      if (currentPrompt) {
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `Failed to update prompt title: ${response.statusText}`
            );
          }
        ...
        prev.map((tab) => {
          if (tab.id === activeTabId) {
            ...
            const variablesChanged =
              JSON.stringify(newVariables) !== JSON.stringify(tab.variables);
            return {
              ...tab,
              variables: newVariables,
              isDirty: tab.isDirty || variablesChanged,
            };
          }
          return tab;
        })
        ...
        prev.map((tab) => {
          if (tab.id === activeTabId) {
            ...
            return {
              ...tab,
              variables: newVariables,
              isDirty: tab.isDirty || variableChanged,
            };
          }
          return tab;
        })
      ...
      .map((element) => {
        if (element.type === "text") {
          ...
          if (
            ...
            element.overrideContent?.includes("{{originalText}}")
          ) {
      ...
      if (over && active.data.current?.type === "library-block") {
        ...
        if (over.id === "text-editor-droppable") {
          ...
          if (over.id === "drop-zone-top") {
            ...
            newOrder =
              sortedContent.length > 0 ? sortedContent[0].order - 1 : 1;
          } else if (afterElementId) {
            // Insert after specific element
            const afterIndex = sortedContent.findIndex(
              (el) => el.id === afterElementId
            );
            if (afterIndex !== -1 && afterIndex < sortedContent.length - 1) {
              ...
              newOrder =
                (sortedContent[sortedContent.length - 1]?.order || 0) + 1;
            }
          } else {
            // Default to end
            newOrder =
          ...
          <div className="flex-1 flex flex-col min-w-0">
            ...
            <main className="flex flex-1 min-h-0">
              ...
              <TextEditorCanvas
                ...
                availableTags={blocks
                  .flatMap((block) => block.tags)
                  .filter((tag, index, arr) => arr.indexOf(tag) === index)}
                availableCategories={blocks
            ...
            <div className="bg-gray-800 border-t border-gray-700 px-6 py-2 text-sm text-gray-400 flex-shrink-0">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <span>
                    {promptContent.length} element
                    {promptContent.length !== 1 ? "s" : ""} in prompt
                  </span>
                  <span>•</span>
                  <span>
                    {promptContent.filter((el) => el.type === "block").length}{" "}
                    block
                    {promptContent.filter((el) => el.type === "block")
                      .length !== 1
                      ? "s"
                      : ""}
                    , {promptContent.filter((el) => el.type === "text").length}{" "}
                    text
                  </span>
                  {Object.keys(variables).length > 0 && (
                    <>
                      ...
                      <span>
                        ...
                        {Object.keys(variables).length !== 1 ? "s" : ""} set
                      </span>
                    </>
                  )}
                </div>
